...
class Client {
    constructor(username = "guest", password = "guest", useWebSockets = false) {
    ...
    _readWebSocketPacket() {
        ...
        while (this.streamWebSocketBuffer.length > sizeTagLength) {
            ...
            try {
                ...
                this._packetReceived(new Packet_1.Packet(FCType, nFrom, nTo, nArg1, nArg2, currentMessage.length, currentMessage.length === 0 ? undefined : currentMessage));
            }
            catch (e) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ensureServerConfigIsLoaded().then(() => {
                if (!this.useWebSockets) {
                    ...
                    Utils_1.logWithLevel(Utils_1.LogLevel.INFO, "Connecting to MyFreeCams websocket server " + chatServer + "...");
                    this.client = new WebSocket(`ws://${chatServer}.myfreecams.com:8080/fcsl`, {
                        origin: "http://m.myfreecams.com",
                    });
                    this.client.on("open", () => {
                        this.client.on("message", (message, flags) => {
                ...
                this.keepAlive = setInterval(function () {
                    ...
                    this.trafficCounter = 0;
                }.bind(this), this.useWebSockets ? 15 * 1000 : 120 * 1000);
            });
        });
    }
    disconnected() {
    ...
    login(username, password) {
        ...
        if (!this.useWebSockets) {
            ...
            this.TxCmd(Constants_1.FCTYPE.LOGIN, 0, 20080909, 0, this.username + ":" + this.password);
        }
    }
