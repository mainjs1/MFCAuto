                ...
                if (packet.nArg2 > 0 && packet.sMessage && packet.sMessage.rdata) {
                    ...
                    switch (nType) {
                        case Constants_1.FCL.ROOMMATES:
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.JOINCHAN, 0, arr.length, 0, undefined));
                            arr.forEach(function (viewer) {
                                _this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.JOINCHAN, packet.nFrom, packet.nTo, packet.sMessage.channel, packet.nArg2, 0, viewer));
                            });
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.JOINCHAN, arr.length, arr.length, 0, undefined));
                            break;
                        case Constants_1.FCL.CAMS:
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.SESSIONSTATE, 0, arr.length, 0, undefined));
                            arr.forEach(function (model) {
                                _this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.SESSIONSTATE, packet.nFrom, packet.nTo, packet.nArg1, model.sid, 0, model));
                            });
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.SESSIONSTATE, arr.length, arr.length, 0, undefined));
                            break;
                        case Constants_1.FCL.FRIENDS:
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.ADDFRIEND, 0, arr.length, 0, undefined));
                            arr.forEach(function (model) {
                            ...
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.ADDFRIEND, arr.length, arr.length, 0, undefined));
                            break;
                        case Constants_1.FCL.IGNORES:
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.ADDIGNORE, 0, arr.length, 0, undefined));
                            ...
                            this._packetReceived(new Packet_1.Packet(Constants_1.FCTYPE.METRICS, packet.nFrom, Constants_1.FCTYPE.ADDIGNORE, arr.length, arr.length, 0, undefined));
                            break;
                        case Constants_1.FCL.TAGS:
                            if (!Array.isArray(rdata)) {
                                ...
                                Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, "[CLIENT] _packetReceived tags list that was an array?: " + JSON.stringify(rdata));
                            }
                            break;
                        default:
                ...
                if (typeof prop === "object") {
                    Object.keys(prop).forEach(function (key) {
                        if (Array.isArray(prop[key])) {
                            ...
                            Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, "[CLIENT] _packetReceived. N-level deep schemas? " + JSON.stringify(schema_1));
                        }
                    });
                }
                else {
            ...
            rdata.forEach(function (record) {
                ...
                for (var i = 0; i < record.length; i++) {
                    if (schemaMap_1.has(i)) {
                        ...
                        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, "[CLIENT] _packetReceived. Not enough elements in schema\n\tSchema: " + JSON.stringify(schema_1) + "\n\tData: " + JSON.stringify(record));
                    }
                }
                result_1.push(msg);
            });
